#!/usr/bin/env python3
"""Accuracy benchmark — 10k × 10k, BM25-only scoring.

First run is cold (builds BM25 index). Subsequent runs are warm.
After matching, evaluates accuracy against ground truth.

Requires: cargo build --release --features bm25

Run from the project root:
    python3 benchmarks/accuracy/10kx10k_bm25/run_test.py
"""
...
TEST_DIR = "benchmarks/accuracy/10kx10k_bm25"
...
def main():
    ...
    print(f"=== Accuracy: BM25-only ({warm}) — {TEST_DIR} ===\n")
