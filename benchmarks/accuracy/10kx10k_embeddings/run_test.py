#!/usr/bin/env python3
"""Accuracy benchmark — 10k × 10k, embeddings-only scoring (usearch).

First run is cold (encodes all records, builds HNSW index). Subsequent runs
are warm (loads cached embeddings + index).
After matching, evaluates accuracy against ground truth.

Requires: cargo build --release --features usearch

Run from the project root:
    python3 benchmarks/accuracy/10kx10k_embeddings/run_test.py
"""
...
TEST_DIR = "benchmarks/accuracy/10kx10k_embeddings"
...
def main():
    ...
    print(f"=== Accuracy: Embeddings-only ({warm}) — {TEST_DIR} ===\n")
