#!/usr/bin/env python3
"""Warm batch benchmark — 100k × 100k, usearch backend.

Preserves the cache; clears outputs only. On first run the cache is empty
so the index will be built (slow, ~3-4 min). On subsequent runs the cache
is reused and the run completes in ~11s. Run from the project root:
    python3 benchmarks/batch/100kx100k_usearch/warm/run_test.py
"""
...
TEST_DIR = "benchmarks/batch/100kx100k_usearch/warm"
