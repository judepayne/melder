#!/usr/bin/env python3
"""Cold batch benchmark — 1M × 1M, usearch backend, production scoring config.

Clears cache and all outputs, then runs a full cold batch job.
Index build will take a very long time at 1M scale (~30+ min on Apple Silicon).
...
Run from the project root:
    python3 benchmarks/batch/1Mx1M_usearch/cold/run_test.py
"""
...
TEST_DIR = "benchmarks/batch/1Mx1M_usearch/cold"
...
def main():
    ...
    print(f"=== Cold batch run (1M x 1M, usearch, production config): {TEST_DIR} ===")
    print("Note: index build will take a very long time at 1M scale.")
    print("Warning: expect ~30-40 GB memory usage.\n", flush=True)
