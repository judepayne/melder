#!/usr/bin/env python3
"""Warm live benchmark — 100k × 100k, usearch + BM25, 10k events at c=10.

Preserves the cache. Clears WAL and crossmap for a clean injection.
On first run the cache is empty so the server builds the index (~3-4 min).
On subsequent runs the server loads from cache and starts in ~1s.
Run from the project root:
    python3 benchmarks/live/100kx100k_inject10k_usearch_bm25/warm/run_test.py
"""
...
TEST_DIR = "benchmarks/live/100kx100k_inject10k_usearch_bm25/warm"
...
def main():
    ...
    if not (os.path.exists(cache_dir) and os.listdir(cache_dir)):
        print("Note: cache is empty — this run will build the index (~3-4 min).")
    ...
    print(f"=== Warm live run (BM25): {TEST_DIR} ===")
