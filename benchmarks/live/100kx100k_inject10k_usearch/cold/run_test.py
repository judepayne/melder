#!/usr/bin/env python3
"""Cold live benchmark — 100k × 100k, usearch backend, 10k events at c=10.

Clears cache, WAL and crossmap, starts the server, waits for it to be
ready (index build takes ~3-4 minutes), injects 10,000 events across
10 concurrent workers, then stops.
Run from the project root:
    python3 benchmarks/live/100kx100k_inject10k_usearch/cold/run_test.py
"""
...
TEST_DIR = "benchmarks/live/100kx100k_inject10k_usearch/cold"
BINARY_DEFAULT = "./target/release/meld"
PORT = 8090
ITERATIONS = 10000
CONCURRENCY = 10
A_DATA = "benchmarks/data/dataset_a_100k.csv"
B_DATA = "benchmarks/data/dataset_b_100k.csv"
SERVER_READY_TIMEOUT = 360  # index build takes ~3-4 min on Apple Silicon
...
def main():
    ...
    print("Starting server (index build takes ~3-4 min on Apple Silicon)...")
