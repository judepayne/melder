...
Try BGE-base (BAAI/bge-base-en-v1.5, 110M params, 768 dims) — 3x the capacity. The hypothesis is that a larger embedding space can learn to separate matches from non-matches without the compression effect seen in BGE-small.

If BGE-base also can't stretch, the path forward is melder's multi-method scoring pipeline: combine embeddings with fuzzy (wratio) and BM25 scoring to cover each method's blind spots.
