//! Encoder pool: N `TextEmbedding` instances behind `Mutex` locks.
//!
//! Each pool slot is an independent ONNX inference session. The pool
//! provides both sync (`encode_blocking`) and async-compatible (`encode`)
//! methods.
